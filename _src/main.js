...
function SlidingPuzzle() {
    ...
    const element = document.getElementById("slidingpuzzle");
    const tiles = {};
...
async function ShinyStars() {
    ...
    const weightedTypes = awa.weightedRandom(types.map((t) => [t.weight, t]), true);
    const stars = [];
    ...
    const loop = new awa.GameLoop({ a: { rate: 20, callback: () => {
                ...
                for (let i = stars.length - 1; i >= 0; i--) {
                    const star = stars[i];
                    if (Date.now() - star.creationTime > star.duration)
                        stars.splice(i, 1);
                }
