...
window.addEventListener("DOMContentLoaded", () => {
    ...
    class VisualLayer {
        ...
        data8 = imageData.data;
        data32 = new Uint32Array(data8.buffer);
        ...
        add(x, y) { this.cells.push(cell); }
        remove(cell) { this.cells.splice(this.cells.indexOf(cell), 1); }
    }
    const thingsToDraw = [];
    function changeThings(iteration) {
    ...
    const perfList = document.getElementById("perf");
    const stepElements = [];
    let totalElement;
    ...
    setInterval(() => {
        ...
        stepElements.forEach((output, index) => {
            ...
            document.getElementById(output.id).value = average.toFixed(2);
            total += average;
        });
