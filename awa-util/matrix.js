...
export class Matrix {
    ...
    getRect(x, y, width, height, options) { return Matrix.getRect(this.array, x, y, width, height, options); }
    static getRect(matrix, x, y, width, height, options) {
        const result = [];
        for (let j = 0; j < height; j++) {
            const row = matrix[y + j];
            if (!row)
                return result; // hit bottom of matrix, end early
            for (let i = 0; i < width; i++) {
                const cell = row[x + i];
                if (cell === undefined)
                    continue;
                if (options?.fill || (j === 0 || j === height - 1 || i === 0 || i === width - 1))
                    result.push(cell);
            }
        }
        return result;
    ...
    static rotate(matrix, angle = 90) {
        ...
        // Validate Angle
        if (angle % 90 !== 0)
            throw new Error("rotate2DArray: Angle must be a multiple of 90 degrees, got " + angle);
