...
class TrackedInput {
  ...
  constructor(element) {
    ...
    this.element.addEventListener("input", (e) => {
      for (const callback of this.callbacks.input) {
        callback(e);
      }
    });
    this.element.addEventListener("change", (e) => {
      for (const callback of this.callbacks.change) {
        callback(e);
      }
    });
  }
...
class InputTracker {
  ...
  static addOutput(output) {
    ...
    if (targetInput === void 0) throw new Error(`No matching input found for name ${output.name}, or it is not yet tracked`);
...
export function trackInputs(identifierClassName = "awa-input") {
  ...
  for (const input of inputElements) {
    if (input.id === void 0) throw new Error("Tracked element must have an id");
  ...
  for (const output of outputElements) {
    if (output.name === void 0) throw new Error("Tracked element must have a specified 'name' attribute");
