...
*/
import { HEIGHT, WIDTH, coordinateGrid } from "./global.js";
// Imports - '@util' in tsconfig.json and vite config
import * as awa from "@util";
import { UpdateLoop } from "@util";
...
function setupLayers() {
    const canvasElements = Array.from(document.getElementById("layers").children);
...
const spawn = (type, x, y, options = {}) => { try {
    return new type(x, y, options);
}
catch { } };
const spawnRandomly = (type, amount = 1, options = {}, attemptCount = 100) => {
    for (let i = 0; i < amount; i++) {
        if (attemptCount < 0)
            return;
        try {
            new type(Math.floor(Math.random() * WIDTH), Math.floor(Math.random() * HEIGHT), options);
        }
        catch {
            spawnRandomly(type, 1, options, attemptCount - 1);
        }
    }
};
...
function displayPerformance(renderLoop, tickLoop) {
    const fpsOutput = document.getElementById("fps");
    const tpsOutput = document.getElementById("tps");
...
function tick(i) {
    ...
    cells.forEach((cell) => {
        if (cell.tick && cell.pauseUntilFrame <= i)
            cell.tick(i);
    });
}
function draw() {
...
function displayRate(getCurrentValue, outputElement, updateIntervalMs = 500, averageOver = 5) {
...
function displayStepPerformance(loops) {
    ...
    setInterval(() => {
        ...
        loops.forEach((loop, i) => {
            ...
            const el = document.getElementById(steps[i].name);
            if (!el)
                return;
